            ...
            <div className="flex flex-wrap items-center justify-between gap-6 mb-6">
              {post.tags && post.tags.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {post.tags.map(tag => (
                    <span key={tag} className="px-3 py-1 bg-[#C2A370]/10 text-[#C2A370] text-[10px] uppercase tracking-widest font-medium rounded-sm">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-6 pt-8 border-t border-white/5">
                {post.tech && post.tech.length > 0 && (
                  <div className="flex-1">
                    ...
                    <ul className="flex flex-wrap gap-2">
                      {post.tech.map(t => (
                        <li key={t} className="px-2 py-1 bg-white/[0.03] border border-white/5 rounded-sm text-[10px] text-white/60">{t}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                {(post.github || post.demo) && (
                  <div className="flex gap-4 items-start">
                    {post.github ? (
                      <a href={post.github} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 transition-colors rounded-sm text-xs font-medium text-white">
